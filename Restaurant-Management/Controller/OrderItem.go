...
func (a *Orderitem_db) CreateOrderItem(c *gin.Context) {
	...
	for _, orderitem := range myorders.Order_Items {
		...
		if validationErr := a.validate.Struct(orderitem); validationErr != nil {
		...
		foodfilter := Food_controll()

		err := foodfilter.foodcollection.FindOne(ctx, bson.M{"food_id": orderitem.Food_id}).Decode(&food)
		...
		var qty = toFixed(*orderitem.Quantity, 2)
...
func (a *Orderitem_db) UpdateOrderitem(c *gin.Context) {
	...
	if orderitem.Unit_price != nil {
		...
		update_order_item = append(update_order_item, bson.E{"unit_price", orderitem.Unit_price})
	}
